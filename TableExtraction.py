...
class TableExtractor():
    ...
    def plot_TD(self, model, pil_img, prob, boxes, delta_xmin, delta_ymin, delta_xmax, delta_ymax):
        ...
        for p, (xmin, ymin, xmax, ymax) in zip(prob, boxes.tolist()):
            cl = p.argmax()
            xmin_pad, ymin_pad, xmax_pad, ymax_pad = xmin - \
                delta_xmin, ymin-delta_ymin, xmax+delta_xmax, ymax+delta_ymax
            # red box shows model bounding box
            ax.add_patch(plt.Rectangle((xmin, ymin), xmax - xmin, ymax -
                         ymin, fill=False, color='red', linewidth=1, linestyle='-.'))
            # blue line is after padding
            ax.add_patch(plt.Rectangle((xmin_pad, ymin_pad), xmax_pad - xmin_pad,
                         ymax_pad - ymin_pad, fill=False, color='blue', linewidth=1, linestyle='--'))

            text = f'{model.config.id2label[cl.item()]}: {p[cl]:0.2f}'
            ax.text(xmin-20, ymin-50, text, fontsize=10,
    ...
    def plot_TSR(self, model, pil_img, prob, boxes, tsr_fontsize=10):
        ...
        for p, (xmin, ymin, xmax, ymax) in zip(prob, boxes.tolist()):
            ...
            if class_text == 'table row':
                if idx % 2 == 0:  # even
                    ax.add_patch(plt.Rectangle((xmin, ymin), xmax - xmin, ymax - ymin,
                                 fill=False, color=self.colors[cl.item()], linewidth=1, linestyle='--'))
                else:  # odd
                    ax.add_patch(plt.Rectangle((xmin, ymin), xmax - xmin, ymax - ymin,
                                 fill=False, color=self.colors[cl.item()], linewidth=1, alpha=0.8))
                text = f'row: {p[cl]:0.2f}'
                ax.text(xmin-10, ymin-10, text, fontsize=tsr_fontsize, bbox=dict(
                    facecolor=self.colors[cl.item()], alpha=0.2), ha='right', rotation=-30)
            ...
            if class_text == 'table column':
                ...
                ax.text(xmin-10, ymin-10, text, fontsize=tsr_fontsize, bbox=dict(
                    facecolor=self.colors[cl.item()], alpha=0.2), ha='left', rotation=30)
        idx += 1
        return TSRfig
    ...
    def crop_rows_cols(self, pil_img, rows: dict, cols: dict):
        ...
        for k, v in rows.items():
            ...
            cropped_img = pil_img.crop(
                (xmin, ymin, xmax, ymax))  # crop the row
            rows[k] = xmin, ymin, xmax, ymax, cropped_img

        for k, v in cols.items():
            xmin, ymin, xmax, ymax = v
            cropped_img = pil_img.crop(
                (xmin, ymin, xmax, ymax))  # crop the column
        ...
        for k_row, v_row in new_master_row.items():
            ...
            for idx, kv in enumerate(new_cols.items()):
                ...
                if idx == 0:  # at the first column, crop at the left edge of row_img
                    cell_xmin = 0
                # at the last column, crop at the right edge of row_img
                ...
                row_img_cropped = row_img.crop(
                    (cell_xmin, cell_ymin, cell_xmax, cell_ymax))  # the actual cropping
    ...
    def create_dataframe(self, ocr_text_list: list, max_rows: int, max_cols: int, first_row_header=False):
        ...
        if first_row_header:  # use first row as header
            ...
            df = pd.DataFrame("", index=range(
                0, max_rows-1), columns=new_headers)
    ...
    def run_extraction(self, image_path: str, cell_img_dir: str, TD_THRESHOLD=0.6, TSR_THRESHOLD=0.8,
        ...
        if show_plots:
            self.plot_TD(model, image, probas, bboxes_scaled,
                         delta_xmin, delta_ymin, delta_xmax, delta_ymax)

        TD_plot = self.plot_TD(model, image, probas, bboxes_scaled,
                               delta_xmin, delta_ymin, delta_xmax, delta_ymax)
        ...
        for table in cropped_img_list:
            ...
            rows, cols = self.generate_structure(
                model, table, probas, bboxes_scaled)

            if show_plots:
                self.plot_TSR(model, table, probas, bboxes_scaled,
                              tsr_fontsize=tsr_fontsize)
            TSR_plot = self.plot_TSR(
                model, table, probas, bboxes_scaled, tsr_fontsize=tsr_fontsize)
            ...
            cells_img, max_cols, max_rows = self.features_to_cells(
                master_row, cols)
            ...
            print(table_info)
            st.markdown(
                f'**{len(rows)}** rows x **{len(cols)}** columns = **{n_cells}** cells')
            ...
            if ocr_choice == 'Google-OCR':
                ...
                for k, img_list in cells_img.items():
                    ...
                    for i, img in enumerate(img_list):
                        ...
                        if status_text is not None:  # output for streamlit app
                            status_text.caption(
                                'Processing cell #: {}'.format(cell_counter))
                        ...
                        google_ocr_texts.append(google_ocr_image_to_text(
                            cell_img_dir + cell_name, CREDENTIALS))
                        # text_google_ocr = google_ocr_image_to_text(cell_img_dir + cell_name, CREDENTIALS)
                    j += 1

                st.subheader(
                    ':arrow_left: Download csv files in the next page')

                table_df = self.create_dataframe(sequential_cell_img_list, len(
                    rows), len(cols), first_row_header=first_row_header)
                ...
                print('PyTesseract OCR process took {:.2f} seconds'.format(
                    time()-start))
